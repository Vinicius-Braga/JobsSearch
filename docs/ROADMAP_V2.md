...
- [x] **Dockerizado**: `Dockerfile` multi-stage (build com JDK, runtime com JRE só) e `docker-compose.yml`. Banco continua sendo o Postgres externo (Supabase) via `.env` — o compose não sobe banco nenhum, só a aplicação. `empresas.txt`/`filtro.txt` montados como volume pra editar sem rebuild. Testado: `docker compose up --build` builda e sobe o container, conecta no Postgres real, e responde `/login` corretamente.
